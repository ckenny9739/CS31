...
void Snake::move()
{
    ...
    {
        case UP:
            if (m_row > 1)                              // Not in top row
                m_row--;
            break;
        case DOWN:                                      // Not in bottom row
            if(m_row < m_pit->rows())
                m_row++;
            break;
        case LEFT:                                      // Not in top col
            if(m_col > 1)
                m_col--;
            break;
        case RIGHT:                                     // Not in bottom col
            if(m_col < m_pit->cols())
                m_col++;
            break;
    }
...
void Player::move(int dir)
{
    ...
    {
        case UP:
            if (m_row - 1 > 0 && m_pit->numberOfSnakesAt(m_row - 1, m_col) == 0)    // Move
                m_row--;
            else if (m_row - 2 > 0)                                             // Jump over snake + kill
            {
            ...
            break;
        case DOWN:
            if (m_row + 1 <= m_pit->rows() && m_pit->numberOfSnakesAt(m_row + 1, m_col) == 0)   // Move
                m_row++;
            else if (m_row + 2 <= m_pit->rows())                                // Jump over snake + kill
            {
                if (m_pit->numberOfSnakesAt(m_row + 2, m_col) != 0)
                    setDead();
                m_pit->destroyOneSnake(m_row + 1, m_col);
            ...
            break;
        case LEFT:
            if (m_col - 1 > 0 && m_pit->numberOfSnakesAt(m_row, m_col - 1) == 0)    // Move
                m_col--;
            else if (m_col - 2 > 0)                                             // Jump over snake + kill
            {
            ...
            break;
        case RIGHT:
            if (m_col + 1 <= m_pit->cols() && m_pit->numberOfSnakesAt(m_row, m_col + 1) == 0)   // Move
                m_col++;
            else if (m_col + 2 <= m_pit->cols())                                // Jump over snake + kill
            {
                if (m_pit->numberOfSnakesAt(m_row, m_col + 2) != 0)
                    setDead();
                m_pit->destroyOneSnake(m_row, m_col + 1);
...
{
    // Counts number of snakes at a position
    int num = 0;
    for (int k = 0; k < m_nSnakes; k++)
...
void Pit::display(string msg) const
{
    ...
    for (r = 0; r < rows(); r++)
        for (c = 0; c < cols(); c++)
        {
            ...
            int numSnake = numberOfSnakesAt(r+1, c+1);
            if (numSnake == 0)
                continue;
            else if (numSnake == 1)
                gridChar = 'S';
            else if (numSnake < 9)
                gridChar = '0' + numSnake;          // Will assign number of snakes as a char
            else
                gridChar = '9';
        }
...
{
    // If no snakes at position, do nothing
    if (numberOfSnakesAt(r, c) == 0)
        return false;
    
    int numOfSnake = 0;
    
    // Find snake in m_nSnakes at position
    for (int k = 0; k < m_nSnakes; k++)
        if (m_snakes[k]->row() == r && m_snakes[k]->col() == c)
        {
            numOfSnake = k;
    ...
    // Destroy snake
    delete m_snakes[numOfSnake];
    
    // Move rest of snakes to appropriate places in array
    for (int k = numOfSnake; k < m_nSnakes; k++)
...
bool Pit::moveSnakes()
{
    for (int k = 0; k < m_nSnakes; k++)
    {
        ...
        // If snake is in same place as player, player is now dead
